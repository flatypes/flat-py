...
__all__ = ['none_tree', 'mk_eq', 'mk_attr', 'mk_call', 'mk_lambda', 'mk_assign', 'mk_foreach', 'mk_import_from',
           'mk_runtime', 'erase_ann', 'get_type_args', 'get_left_values', 'get_operands', 'is_ellipsis', 'is_pure']

none_tree = ast.Constant(None)
...
def mk_runtime(value: object, rt: str) -> ast.expr:
    ...
    if isinstance(value, Sequence):
        return ast.List([mk_runtime(elem, rt) for elem in value])

    if isinstance(value, Mapping):
        return ast.Dict([mk_runtime(k, rt) for k in value.keys()],
                        [mk_runtime(v, rt) for v in value.values()])

    if is_dataclass(value):
        if type(value).__module__ in ('flat.py.shared', 'flat.py.runtime'):
            constr = rt + '.' + type(value).__qualname__
            args = astuple(value)  # type: ignore
            return mk_call(constr, *[mk_runtime(arg, rt) for arg in args])
